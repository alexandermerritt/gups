...
int nsend,nkeep,nrecv;
  ...
  const uint flags = MAP_PRIVATE|MAP_ANON|MAP_NOCORE|MAP_NOSYNC;
  const uint prot = PROT_READ|PROT_WRITE;
  const size_t len = (nlocal * sizeof(u64Int));
  const size_t chunk = (CHUNKBIG*sizeof(u64Int));
  table = mmap(NULL, len, prot, flags, -1, 0);
  if (table == MAP_FAILED)
      MPI_Abort(MPI_COMM_WORLD,1);
  data = mmap(NULL, chunk, prot, flags, -1, 0);
  if (table == MAP_FAILED)
      MPI_Abort(MPI_COMM_WORLD,1);
#ifdef SET_COLORS
  if (mcontrol(table, len, MADV_PGCOLOR, 1)) {
      perror("mcontrol");
      MPI_Abort(MPI_COMM_WORLD,1);
  }
  if (mcontrol(data, chunk, MADV_PGCOLOR, 1)) {
      perror("mcontrol");
      MPI_Abort(MPI_COMM_WORLD,1);
  }
#endif
  printf("%6lf GB table\n", (nlocal*8)/(float)(1<<30));
  printf("%6f MB data\n", (CHUNKBIG*8)/(float)(1<<20));
...
#else
  send1 = mmap(NULL, chunk, prot, flags, -1, 0);
  send2 = mmap(NULL, chunk, prot, flags, -1, 0);
  send = send1;
  if (send1 == MAP_FAILED || send2 == MAP_FAILED) {
    if (me == 0) printf("Table allocation failed\n");
    MPI_Abort(MPI_COMM_WORLD,1);
  }
#ifdef SET_COLORS
  if (mcontrol(send1, chunk, MADV_PGCOLOR, 1)) {
      perror("mcontrol");
      MPI_Abort(MPI_COMM_WORLD,1);
  }
  if (mcontrol(send2, chunk, MADV_PGCOLOR, 1)) {
      perror("mcontrol");
      MPI_Abort(MPI_COMM_WORLD,1);
  }
#endif
  ...
  for (j = 0; j < PITER; j++)
    for (i=0; i<logprocs; i++) {
      recv[j][i] = mmap(NULL, RCHUNK*sizeof(u64Int), prot, flags, -1, 0);
      if (recv[j][i] == MAP_FAILED) {
  ...
  memset(data, 0, chunk);
  printf("# send1 %p\n", send1);
  memset(send1, 0, chunk);
  printf("# send2 %p\n", send2);
  memset(send2, 0, chunk);

  int fd = open("/proc/curproc/pmap", O_RDONLY);
  ...
  while (read(fd, buf, 16384) > 0)
      printf("%s", buf);
  printf("\n"); fflush(stdout);
  close(fd);

  open("/proc/curproc/map", O_RDONLY);
  if (fd < 0)
      MPI_Abort(MPI_COMM_WORLD,1);
  while (read(fd, buf, 16384) > 0)
      printf("%s", buf);
